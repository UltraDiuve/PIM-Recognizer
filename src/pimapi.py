...
class Requester(object):
    ...
    def fetch_modified_files(self, batch_size=100, **kwargs):
        ...
        perrave.
        """
        raise DeprecationWarning('Not implemented anymore !')
        self._load_directory()
        df = self._directory
        mask = (df.lastFetchedFiles.isna() |
                (df.lastModified > df.lastFetchedFiles))
    ...
    def fetch_modified(self, what={'files'}):
        """DEPRECATED !!! Fetches the data and/or files from source
    ...
    def get_info_from_uid(self, uid, nx_properties='*'):
        """DEPRECATED !!!Requests an object data from PIM

        nx_properties describes which schemes are to be retrieved. Setting it
        to '*' means all data. Setting it to '' returns only Nuxeo standard
        data.
        """
        raise DeprecationWarning('Not implemented anymore !!!')
        headers = {'Content-Type': 'application/json',
    ...
    def _set_dump_path(self):
        """DEPRECATED !!!! Computes and sets inner dump path attribute
